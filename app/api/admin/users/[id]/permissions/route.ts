...
export const GET = withSuperAdmin(async (req: Request) => {
  ...
  const [assignRes, permRes] = await Promise.all([
    supabase.from('user_client_assignments').select('client_id').eq('user_id', userId),
  ...
  return apiSuccess({
    assigned_client_ids: (assignRes.data || []).map((d: any) => d.client_id),
...
export const PUT = withSuperAdmin(async (req: Request) => {
  ...
  const { assigned_client_ids, menu_permissions } = await req.json()
  const supabase = serverSupabase()
  ...
  if (Array.isArray(assigned_client_ids)) {
    ...
    if (assigned_client_ids.length > 0) {
      const rows = assigned_client_ids.map((client_id: number) => ({ user_id: userId, client_id }))
      const { error } = await supabase.from('user_client_assignments').insert(rows)
      if (error) return apiError('클라이언트 배정 저장 실패: ' + error.message, 500)
    }
  }
