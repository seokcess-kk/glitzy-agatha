...
export default function InvoiceList({ clientId }: InvoiceListProps) {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/erp-documents?type=invoices&client_id=${clientId}&page=${page}&limit=20`
      )
  ...
  const fetchDetail = async (id: string) => {
    ...
    try {
      const res = await fetch(
        `/api/erp-documents/${id}?type=invoices&client_id=${clientId}`
      )
      const json = await res.json()
      if (json.success) {
        setDetail(json.data)
    ...
    return (
      <Card variant="glass" className="p-4 md:p-5">
        <EmptyState
          icon={Receipt}
          title="계산서가 없습니다"
          description="이 클라이언트에 등록된 계산서가 없습니다."
        />
      </Card>
    )
  }
